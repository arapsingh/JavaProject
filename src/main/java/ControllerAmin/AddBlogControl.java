		...
		try {
			...
			for (FileItem item : items) {
				if (item.getFieldName().equals("title")) {
					blog.setTitle(item.getString("UTF-8"));
					...
					blog.setContent1(item.getString("UTF-8"));
				} else if (item.getFieldName().equals("ct2")) {
					blog.setContent2(item.getString("UTF-8"));
				} else if (item.getFieldName().equals("ct3")) {
					blog.setContent3(item.getString("UTF-8"));
				} else if (item.getFieldName().equals("ct4")) {
					blog.setContent4(item.getString("UTF-8"));
				}
			}
